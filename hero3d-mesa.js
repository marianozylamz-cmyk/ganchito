/* =========================================================
   ...
   hero3d-mesa.js
   Geometría + materiales de la mesa protagonista. Copia directa del
   motor ya probado en experimental-3d/hero-mesa.js — no depende de
   ganchito-configurador.js (que queda desconectado de esta sección).

   Cada página que use este motor define `window.HERO_CONFIG` ANTES de
   este script con { tipo, color, patron, ... }.
   ========================================================= */
...
window.HeroMesa = (function () {
  ...
  function buildRatonaMadera(group, w, d, h, woodMat) {
    ...
    // Solo las 4 patas — sin travesaños horizontales.
  ...
  function buildRatonaMetal(group, w, d, h) {
    ...
    // Solo las 4 patas (cada una, dos varillas en diagonal tipo
    // "hairpin") — sin travesaños horizontales entre patas.
  ...
  // Reconstruye la mesa con OTRO tipo, conservando color/patrón/tamaño
  // elegidos — la usa el switcher para cambiar de mesa sin recargar la
  // página. Descarta geometrías/texturas viejas para no filtrar memoria.
  function rebuild(scene, renderer, tipo) {
    if (mesaGroup) {
      scene.remove(mesaGroup);
      disposeGroup(mesaGroup);
    }
    currentTipo = tipo;
    return build(scene, renderer);
  }
